...
import {
  extractJsonFromMarkdownBlock,
  retrieveDataBasedOnPrompt,
} from "../lib/utils";
import {
...
import {
  filterDappsByCategories,
  retrieveFinalRecommendation,
...
export const getAppsInfoAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      const lastUserMessage = recentUserMessages.find(
        (mem) => mem.entityId !== runtime.agentId
      );
      ...
      callback({
        text: "User retrieved data: " + JSON.stringify(parsedUserIntent),
        actions: ["GET_APPS_INFO"],
        source: message.content.source,
      });
      ...
      const fetchedDappsViaApi =
        state.data.providers.appsProvider.data.fetchedDappsViaApi;
      ...
      const dappsWithRatings = combineDappWithRatings(
      ...
      if (recommendationResult && typeof recommendationResult === "object") {
        ...
        const formattedRecommendation =
          formatRecommendationsForDisplay(finalRecommendation);
